...
export const NXA_ENERGY_ORB_CONFIGURABLE_FRAGMENT_SHADER = [
  ...
  "uniform vec2 uMouse;uniform float uHover;uniform float uPulse;uniform vec2 uClick;",
  "float hash(vec3 p){p=fract(p*0.3183099+vec3(0.1,0.2,0.3));p*=17.0;return fract(p.x*p.y*p.z*(p.x+p.y+p.z));}",
  "float noise(vec3 x){vec3 i=floor(x);vec3 f=fract(x);f=f*f*(3.0-2.0*f);",
  " return mix(mix(mix(hash(i),hash(i+vec3(1,0,0)),f.x),mix(hash(i+vec3(0,1,0)),hash(i+vec3(1,1,0)),f.x),f.y),",
  " mix(mix(hash(i+vec3(0,0,1)),hash(i+vec3(1,0,1)),f.x),mix(hash(i+vec3(0,1,1)),hash(i+vec3(1,1,1)),f.x),f.y),f.z);}",
  ...
  "void main(){",
  " vec2 uv=(gl_FragCoord.xy-0.5*uR)/min(uR.x,uR.y);",
  " vec2 aspect=uR/min(uR.x,uR.y);",
  " uv-=uMouse*0.06*aspect;",
  " float r=length(uv);",
  " float R=0.31*(1.0+uHover*0.05+uPulse*0.09);",
  " float ring=abs(length(uv-(uClick*0.5*aspect))-(1.0-uPulse)*0.9);",
  " float ripple=uPulse*smoothstep(0.05,0.0,ring)*0.6;",
  " vec3 col=vec3(0.0);float alpha=0.0;",
  " if(r<R){",
  "   float z=sqrt(R*R-r*r);",
  "   vec3 n=normalize(vec3(uv,z));",
  "   float ca=uT*0.15+uMouse.x*0.9;",
  "   float cb=-uMouse.y*0.6;",
  "   mat3 rot=mat3(cos(ca),0.,sin(ca),0.,1.,0.,-sin(ca),0.,cos(ca));",
  "   mat3 tilt=mat3(1.,0.,0.,0.,cos(cb),-sin(cb),0.,sin(cb),cos(cb));",
  "   vec3 sp=tilt*rot*n;",
  "   float smokeTime=uT*uSmokeSpeed*(1.0+uHover*0.8+uPulse*2.0);",
  ...
  "   float fres=pow(1.0-z/R,2.2);",
  "   col+=vec3(0.55,0.55,1.0)*fres*1.1*uGlow*(1.0+uHover*0.35);",
  "   vec3 lightDir=normalize(vec3(uMouse.x*0.8,0.7+uMouse.y*0.6,0.7));",
  "   float top=pow(max(dot(n,lightDir),0.0),3.0);",
  "   col+=vec3(0.45,0.42,0.9)*top*(0.35+uHover*0.3)*uGlow;",
  "   col+=vec3(0.7,0.9,1.0)*uPulse*uPulse*0.5;",
  "   alpha=1.0;",
  " }",
  " float glow=exp(-(r-R)*(14.0-uHover*3.0-uPulse*4.0));",
  " if(r>=R){",
  "   glow=clamp(glow,0.0,1.0);",
  "   col=vec3(0.55,0.52,1.0)*glow*(0.8+uHover*0.25)*uGlow;",
  "   alpha=glow*0.85;",
  " } else {",
  "   float rim=smoothstep(R-0.03,R,r);",
  "   col+=vec3(0.6,0.58,1.0)*rim*0.6*uGlow;",
  " }",
  " col+=vec3(0.45,0.8,1.0)*ripple;",
